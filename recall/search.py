...
def _terms_from_run(run: str) -> list[str]:
    ...
    （上流 agent-shelf レビュー指摘: 空白分割のまま日本語がヒットしない）を避ける。
    どちらの種別も3文字未満は trigram tokenizer で照合不能なため捨てる。
    """
...
def build_fts_query(question: str) -> str:
    ...
    ストライドサンプリングで選ぶ（上流 agent-shelf レビュー指摘: 長い質問文の
    先頭切り詰めで後半の重要語が反映されない）。
    """
    terms: list[str] = []
    run_chars: list[str] = []
